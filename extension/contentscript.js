...
var makeIcs = function(startDate, endDate, breakStartDate, weekEvents) {
	// parameter defaults
	...
	var breakStartingDate = new Date("6 April, 2015");
	...
	// move to the end if one exists
	var sunday = $(daySelector+" .cssTtbleColHeaderInner :contains('Sunday')");
	if (sunday.length > 0) {
		var sundayCol = sunday.closest(daySelector);
		sundayCol.parent().append(sundayCol);
	}
	...
	$(daySelector).each(function(index, element){
		...
		var exludeDay = new Date(breakStartingDate.getTime() + dayIndex*(24 * 60 * 60 * 1000));
	...
	if (weekEvents) {
		...
		for(var weeknum=1;weeknum<=13;weeknum++) {
			...
			if (weekStartDate.toDateString() == breakStartingDate.toDateString()) {
